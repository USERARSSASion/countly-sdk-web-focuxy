        ...
        **/
        ...
        function processWidget(currentWidget, hasSticker) {
            ...
            try {
                ...
                iframe.src = self.url + "/feedback?widget_id=" + currentWidget._id + "&app_key=" + self.app_key + '&device_id=' + self.device_id + '&sdk_version=' + SDK_VERSION + '&path=' + baseUrlPath + '&target=' + target;
                // inject them to dom
                document.body.appendChild(wrapper);
                ...
                if (hasSticker) {
                    ...
                    var appendArr = ['HelpDetail', 'snkrs'];
                    var appendReg = new RegExp(_.chain(appendArr).uniq().join('|').valueOf())
                    var list = location.href.match(appendReg);
                    if(!!list) {
                        var langSet = localStorage.getItem('langSet');
                        stickerText.innerText = langSet && langSet === 'zh' ? '有' : 'YES';
                        document.getElementById('markdownBox').appendChild(sticker);
                    } else {
